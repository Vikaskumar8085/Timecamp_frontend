...
const ContractorTaskInfo = () => {
  ...
  const fetchtaskinfofunc = async () => {
    try {
      const response = await apiInstance.get(
        `/v2/contractor/fetch-contractor-single-task/${id}`
      );
  ...
  const fetchtaskproject = async () => {
    try {
      const response = await apiInstance.get(
        `/v2/contractor/fetch-contractor-single-project/${projectids}`
      );
      if (response?.data?.success) {
    ...
    onSubmit: async (values) => {
      try {
        const response = await apiInstance.put(
          `/v2/contractor/update-task-progress/${id}`,
          values
        );
        if (response?.data?.success) {
          setOpen(false);
...
export default ContractorTaskInfo;
